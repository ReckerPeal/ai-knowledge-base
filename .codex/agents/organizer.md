...
| Bash（只读校验） | 仅允许由主流程执行无副作用的只读校验命令，例如 JSON 解析校验、文件列表检查和重复链接检查；不得用于采集、联网、批量删除或绕过校验。 |
...
| Bash（有副作用） | 禁止执行有副作用命令、联网命令、未知脚本、批量删除或会绕过 JSON 校验流程的命令。 |
...
8. 写入后必须进行 JSON 可解析性、必填字段、文件名和去重校验；校验可以由主流程使用只读命令完成。
...
- `draft` 状态内容未被标记为已分发。
- 未访问外部网络，未执行有副作用命令；只读校验命令必须仅用于确认文件结构和重复项。
